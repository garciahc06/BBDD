...
/*
Escribe una función para la base de datos tienda que devuelva el valor mínimo del precio de los productos de un determinado fabricante
...
El procedimiento devolverá como salida un parámetro llamado error que tendrá un valor igual a 0 si la operación se ha podido realizar
con éxito y un valor igual a 1 en caso contrario.
        ...
        START TRANSACTION;
            IF ((SELECT COUNT(cuentas.id_cuenta)
               ...
               WHERE cuentas.id_cuenta = id_cuenta) = 0) THEN
                   SELECT 'No se ha podido encontrar la cuenta' as 'ERROR TYPE';
                   ROLLBACK;
            ELSE
                UPDATE cuentas SET saldo = saldo - 5 WHERE cuentas.id_cuenta = id_cuenta;
                INSERT INTO entradas VALUES (id_butaca, nif);
...
CREATE PROCEDURE crear_lista_emails_alumnos(OUT lista VARCHAR(255))
BEGIN
    DECLARE done INT DEFAULT 0;
    DECLARE mail VARCHAR(30);
    DECLARE cur1 CURSOR FOR
...
DELIMITER //
DROP PROCEDURE IF EXISTS crear_email ;
CREATE PROCEDURE crear_email(IN nombre VARCHAR(25), IN apellido1 VARCHAR(25), IN apellido2 VARCHAR(25), IN dominio VARCHAR(25), OUT email VARCHAR(50))
    BEGIN
        SET email = CONCAT(
                                LOWER(LEFT(nombre,1)),
                                LOWER(LEFT(apellido1, 3)),
